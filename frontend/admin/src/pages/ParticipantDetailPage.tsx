...
export function ParticipantDetailPage() {
  ...
  return (
    <div>
      <Link to={`/tests/${testId}/participants`} className="mb-4 inline-block text-sm text-indigo-600 hover:underline">
        ← Все участники
      </Link>

      <div className="mb-6 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <h1 className="text-xl font-semibold text-slate-900">{detail.student.fullName}</h1>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(detail.sectionStats).map(([section, stat]) => {
            ...
            return (
              <div key={section} className="rounded-md border border-slate-200 p-3">
      ...
      <div className="space-y-3">
        {sortedAnswers.map((a) => (
          <div
            key={a.id}
            className={`rounded-lg border p-4 ${a.isCorrect ? "border-emerald-200 bg-emerald-50/40" : "border-red-200 bg-red-50/40"}`}
          >
            <div className="mb-1 text-xs uppercase tracking-wide text-slate-500">
              Q{a.question.order} · {a.question.section}
            </div>
            <p className="mb-2 text-sm font-medium text-slate-900">{a.question.prompt}</p>
